(function(){ try {var elementStyle = document.createElement('style'); elementStyle.appendChild(document.createTextNode(".vue-card{margin-top:1.5rem;margin-left:1.5rem;background-color:#fff;border-radius:.25rem;box-shadow:0 1px 3px #0000001a,0 1px 2px #0000000f;margin-bottom:1.5rem;overflow:hidden;width:40%;display:flex;flex-direction:column;justify-content:center;align-self:center}.vue-card-header{padding:1.25rem 1.5rem;border-bottom:1px solid #e9ecef}.vue-card-header h3{margin-bottom:0;font-size:1.5rem;font-weight:400;line-height:1.5}.vue-card-body{padding:1.25rem 1.5rem}.vue-card-body p{margin-bottom:1.25rem}")); document.head.appendChild(elementStyle);} catch(e) {console.error('vite-plugin-css-injected-by-js', e);} })();var __defProp = Object.defineProperty;
...
function trackRuntimeError(errorObservable) {
  return startUnhandledErrorCollection(function(stackTrace, errorObject) {
    var _a2 = formatUnknownError(stackTrace, errorObject, "Uncaught"), stack = _a2.stack, message = _a2.message, type = _a2.type;
...
function buildRawReportFromReport(_a2) {
  var type = _a2.type, body = _a2.body;
  return {
    type,
    subtype: body.id,
...
function startLogsAssembly(sessionManager, configuration, lifeCycle, getCommonContext, mainLogger) {
  ...
  statusWithCustom.forEach(function(status) {
    logRateLimiters[status] = createEventRateLimiter(status, configuration.eventRateLimiterThreshold, function(error) {
...
const register = (type, fn, module) => {
  const id = `${type}__${Date.now()}__${Math.floor(Math.random() * 1e3)}`;
  let eventType = type;
  if (module) {
...
const replaceModuleKey = (type, module) => {
...
class FederatedRuntime {
  ...
  ensureSystemJs() {
    if (!this.useNativeModules) {
      ...
      addScriptTag("systemjs-named-exports", `${this.sharedDependencyBaseUrl}/systemjs/6.12.1/extras/named-exports.min.js`);
      addScriptTag("systemjs-amd", `${this.sharedDependencyBaseUrl}/systemjs/6.12.1/extras/amd.min.js`);
      addScriptTag("systemjs-dynamic-import-maps", `${this.sharedDependencyBaseUrl}/systemjs/6.12.1/extras/dynamic-import-maps.min.js`);
      this.services.event.emit({
...
const mountLifecycle = (module, federatedRuntime, opts, defaultProps) => {
  ...
  return async (props = defaultProps, mountId) => {
    try {
      const { Vue: Vue2, config: { scope: scope2, name: name2, domElementId } } = opts;
      ...
      if (!domContainer) {
        eventService.emit({
          ...
          payload: {
            module,
            error: new Error(`Could not find dom container with id ${domElementId}`)
          }
        }, module);
      }
      ...
      rootComponent = rootComponent || federatedRuntime.getModuleRootComponent(module);
      const rootComponentElement = Vue2.createApp(rootComponent);
      eventService.emit({
        type: FederatedEvents.MODULE_BEFORE_MOUNT,
        payload: {
          module
      ...
      federatedRuntime == null ? void 0 : federatedRuntime.setModuleState(module, FederatedModuleStatuses.LOAD_ERROR);
    }
  };
};
const unmountLifecycle = (module, federatedRuntime, opts) => {
  try {
    const { Vue: Vue2 } = opts;
    const domElementId = opts.config.domElementId || `${module.scope}-${module.name}`;
...
function createFederatedVue(options) {
  validateModuleOptions(options);
  const { config, federatedRuntime } = options;
  const { rootComponent } = config;
  const { domElementId = `${config.scope}-${config.name}`, loadRootComponent, defaultProps, name: name2, scope: scope2, type, description, propValidationFunction, activeWhenPaths, exceptWhenPaths } = config;
...
var Card_vue_vue_type_style_index_0_lang = /* @__PURE__ */ (() => ".vue-card{margin-top:1.5rem;margin-left:1.5rem;background-color:#fff;border-radius:.25rem;box-shadow:0 1px 3px #0000001a,0 1px 2px #0000000f;margin-bottom:1.5rem;overflow:hidden;width:40%;display:flex;flex-direction:column;justify-content:center;align-self:center}.vue-card-header{padding:1.25rem 1.5rem;border-bottom:1px solid #e9ecef}.vue-card-header h3{margin-bottom:0;font-size:1.5rem;font-weight:400;line-height:1.5}.vue-card-body{padding:1.25rem 1.5rem}.vue-card-body p{margin-bottom:1.25rem}\n")();
...
const _sfc_main = {};
const _hoisted_1 = { class: "vue-card" };
const _hoisted_2 = { class: "vue-card-header" };
const _hoisted_3 = /* @__PURE__ */ createElementVNode("h3", { class: "vue-card-title" }, "Card", -1);
const _hoisted_4 = { class: "vue-card-body" };
const _hoisted_5 = /* @__PURE__ */ createElementVNode("p", null, " I'm a card block created in Vue. Click on the edit button to change this text. ", -1);
function _sfc_render(_ctx, _cache) {
  return openBlock(), createElementBlock("div", _hoisted_1, [
    createElementVNode("div", _hoisted_2, [
      renderSlot(_ctx.$slots, "vue-header", {}, () => [
        _hoisted_3
      ])
    ]),
...
const { name, scope, mount, unmount, bootstrap, update } = createFederatedVue({
  ...
  config: {
    name: "FederatedCard",
    rootComponent: Card,
    scope: "vfuk-federated-vue-example",
    type: "component"
  }
});
export { bootstrap, mount, name, scope, unmount, update };
